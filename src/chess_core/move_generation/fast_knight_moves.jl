...
function generate_knight_moves_fast!(board::Board, moves::Vector{Move})
    # Determine knight, friendly, and enemy bitboards
    if board.side_to_move == WHITE
        knights = board.bitboards[Piece.W_KNIGHT]
        friendly_pieces = [board.bitboards[i] for i in Piece.W_PAWN:Piece.W_KING]
        enemy_pieces = [board.bitboards[i] for i in Piece.B_PAWN:Piece.B_KING]
    else
        knights = board.bitboards[Piece.B_KNIGHT]
        friendly_pieces = [board.bitboards[i] for i in Piece.B_PAWN:Piece.B_KING]
        enemy_pieces = [board.bitboards[i] for i in Piece.W_PAWN:Piece.W_KING]
    end

    # Combine friendly and enemy boards
    occupied_friendly = zero(UInt64)
    for bb in friendly_pieces
        occupied_friendly |= bb
    end
    enemy_board = zero(UInt64)
    for bb in enemy_pieces
        enemy_board |= bb
    end
    ...
    while bb != 0
        ...
        attacks = knight_attack_masks[sq + 1] & ~occupied_friendly
        attack_bb = attacks
        while attack_bb != 0
            to_sq = trailing_zeros(attack_bb)
            attack_bb &= attack_bb - 1

            capture = OrbisChessEngine.testbit(enemy_board, to_sq) ? OrbisChessEngine.piece_at(enemy_pieces, to_sq) : 0
...
function generate_knight_moves_zeroalloc!(board::Board, moves::Vector{Move})
    # Determine knight, friendly, and enemy bitboards
    if board.side_to_move == WHITE
        knights = board.bitboards[Piece.W_KNIGHT]
    ...
    while bb != 0
        ...
        while attack_bb != 0
            ...
            # Find which piece is captured using bitwise ANDs directly
            capture = 0
