...
function generate_king_moves_zeroalloc!(board::Board, moves::Vector{Move})
    ...
    if board.side_to_move == WHITE
        if (rights & 0b0001) != 0 &&
           ( (board.occupied & ((UInt64(1) << 5) | (UInt64(1) << 6))) == 0 )
            push!(moves, Move(4, 6; castling=1))
        end
        if (rights & 0b0010) != 0 &&
           ( (board.occupied & ((UInt64(1) << 1) | (UInt64(1) << 2) | (UInt64(1) << 3))) == 0 )
            push!(moves, Move(4, 2; castling=2))
        end
    else
        if (rights & 0b0100) != 0 &&
           ( (board.occupied & ((UInt64(1) << 61) | (UInt64(1) << 62))) == 0 )
            push!(moves, Move(60, 62; castling=1))
        end
        if (rights & 0b1000) != 0 &&
           ( (board.occupied & ((UInt64(1) << 57) | (UInt64(1) << 58) | (UInt64(1) << 59))) == 0 )
