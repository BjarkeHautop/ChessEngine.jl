...
@testset "Polyglot position hash correct" begin
    ...
    b = board_from_fen("8/8/8/8/8/8/8/K7 b - - 0 1")
    h = polyglot_hash(b)

    @test h == 0x55b6344cf97aafae

    b = board_from_fen("8/8/8/8/8/8/8/K1k5 b - - 0 1")
    h = polyglot_hash(b)

    @test h == 0x6f3e94b745caf3cd

    b = board_from_fen("8/8/8/8/8/8/8/K1k5 w - - 0 1")
    h = polyglot_hash(b)

    @test h == 0x97e8b21deaed76c4

    b = board_from_fen("4k3/8/8/8/8/8/8/4K2R w - - 0 1")
    h = polyglot_hash(b)

    @test h == 0x8f08c83346abde2c

    b = board_from_fen("4k3/8/8/8/8/8/8/4K2R b K - 0 1")
    h = polyglot_hash(b)
